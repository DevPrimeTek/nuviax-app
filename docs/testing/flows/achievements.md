# docs/testing/flows/achievements.md — Achievement & Ceremony Flow

> Part of: `docs/testing/` | Scenarios: `scenarios/critical.md` (TS-07)
...
**⚠️ NOT IMPLEMENTED — Achievement auto-award (SA-2):** `fn_award_achievement_if_earned()` exists in migration 006 but is never called from Go. `achievement_badges` is not populated by the scheduler. `GET /achievements` always returns `[]` for real users. Badges only appear if inserted directly via DB.
